...
class ExportQuakeMap(bpy.types.Operator, ExportHelper):
    ...
    def texdata(self, face, mesh, obj, orig_obj):
        ...
        # # Alternatively, use uniform scale if you prefer
        # # scale = obj.get("tex_scale", 1.0)
        # # scale_x = scale
        # # scale_y = scale

        # # --- Apply your existing transform conversions ---
        # # Rotation: multiply by 60 and negate (your existing formula)
        # if rotation != 0.0:
        ...
        # # Offset: multiply by 10
        # if offset_x != 0.0:
            # offset_x *= 10.0
        # if offset_y != 0.0:
            # offset_y *= 10.0

        # # Convert scale for texture size (standard Radiant)
...
def add_convex_hull_node(node_group):
    node_group.interface.new_socket("Scale", socket_type="NodeSocketFloat", in_out='INPUT')
    ...
    node_group.links.new(combine_node1.outputs['Vector'], mix_node1.inputs['A'])
    node_group.links.new(combine_node2.outputs['Vector'], mix_node1.inputs['B'])
    node_group.links.new(combine_node3.outputs['Vector'], mix_node2.inputs['B'])
...
class ConvexHullBrush(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.scene.trenchcoat_props.automerge:
            ...
            active_object.modifiers["ConvexHullBrush"]["Socket_2"] = 1.0
            ...
            for obj in selected_objects:
                ...
                obj.modifiers["ConvexHullBrush"]["Socket_2"] = 1.0
...
class FillWithBrush(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.mode == 'OBJECT':
            ...
            new_obj.modifiers["ConvexHullBrush"]["Socket_2"] = 1.0
            new_obj.name = 'brush'              
