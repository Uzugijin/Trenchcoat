...
class Trenchcoat_Properties(bpy.types.PropertyGroup):

    text_of_prop : bpy.props.StringProperty(name="", default="", description="commands:\n[<key>, <value>] to set key and value pairs\n[<key>, (<key>)] to assign other key's value\n[!<key>, value] sets key and value on all selected entities\n[del <key>, <key2>, etc.] to delete keys\n[del all] to delete all keys\n[get <key>] to return key's value into the text box\n[target<- <name>] to set target and targetname on selected objects, target being self.\n[target-> <name>] to set target and targetname on selected objects, target being others\n[? <name>] select referenced entities\n[?? <name>] select similar purpose entities")
    snapset1 : bpy.props.BoolProperty(name="Snapset1", default=False, description="Snapping set 1")
    snapset2 : bpy.props.BoolProperty(name="Snapset2", default=False, description="Snapping set 2")
    snapset3 : bpy.props.BoolProperty(name="Snapset3", default=False, description="Snapping set 3")

    numberofverts : bpy.props.IntProperty(name="Count", default=2, description="Max vertex count to check", min=0, max=3)

    gridsteps : bpy.props.FloatProperty(name="Steps", default=5.0, description="Steps to take")
    automerge : bpy.props.BoolProperty(name="Merge Selected", default=True, description="Merge all selected objects into one brush")
    snap : bpy.props.BoolProperty(name="Snap to Grid", default=True, description="Automatically snaps to grid on brush conversion. Snapping ensures best conversion, but distorts rotation of mesh")
    invasive_mirror : bpy.props.BoolProperty(name="Use Active Object", default=True, description="Uses the active object's bounding box to set new origins")
    snap_alone : bpy.props.BoolProperty(name="Only selected", default=False, description="Snap only the selected vertices")
    mirror_axis : bpy.props.EnumProperty(name="Mirror Axis", description="Which axis to mirror onto", items=[("x", "X", ""),("y", "Y", ""), ("z", "Z", "")], default="x")
    shrt_xray : bpy.props.BoolProperty(name="X-Ray", default=False, description="X-Ray")
    shrt_obj_col : bpy.props.BoolProperty(name="Object Color", default=False, description="Object Color")
    material : bpy.props.PointerProperty(type=bpy.types.Material)
    shrt_pivot_cursor : bpy.props.BoolProperty(name="Pivot Cursor", default=False, description="Pivot Cursor")
    frame_object : bpy.props.PointerProperty(type=bpy.types.Object)
    grid_size : bpy.props.EnumProperty(
        name="Grid Size",
...
class OBJECT_OT_convcol_suffix(bpy.types.Operator):
    """Add or remove .convcol suffix from objects with 'brush' in their name"""
    bl_idname = "object.convcol_suffix"
    bl_label = "Convcol Suffix"
    bl_options = {'REGISTER', 'UNDO'}

    action: bpy.props.EnumProperty(
        name="Action",
        description="What to do with the suffix",
        items=[
            ('TOGGLE', "Toggle", "Toggle .convcol suffix on/off"),
            ('ADD', "Add", "Force add .convcol suffix to all brush objects"),
            ('REMOVE', "Remove", "Force remove .convcol suffix from all brush objects"),
        ],
        default='TOGGLE'
    )

    def remove_convcol_suffix(self, name):
        """Remove .convcol suffix from a name, handling multiple instances"""
        # Split on .convcol and take the first part
        parts = name.split('.convcol')
        return parts[0]

    def execute(self, context):
        # Find all objects with 'brush' in their name (case insensitive)
        brush_objects = [obj for obj in context.scene.objects if 'brush' in obj.name.lower()]

        if not brush_objects:
            self.report({'WARNING'}, "No objects with 'brush' in name found")
            return {'CANCELLED'}
        ...
        for obj in brush_objects:
            has_suffix = '.convcol' in obj.name

            if self.action == 'TOGGLE':
                if has_suffix:
                    new_name = self.remove_convcol_suffix(obj.name)
                    obj.name = new_name
                    restored_count += 1
                else:
                    obj.name = obj.name + '.convcol'
                    renamed_count += 1
        ...
        # Report results
        if renamed_count > 0:
            self.report({'INFO'}, f"Added .convcol to {renamed_count} objects")
        if restored_count > 0:
            self.report({'INFO'}, f"Removed .convcol from {restored_count} objects")
        if unchanged_count > 0:
            self.report({'INFO'}, f"{unchanged_count} objects were unchanged")
...
class ExportQuakeMap(bpy.types.Operator, ExportHelper):
    ...
    def texdata(self, face, mesh, obj, orig_obj):
        ...
        # # --- Get texture parameters from object custom properties ---
        # Get rotation (single value for triplanar)
        rotation = obj.get("tex_rotation", 0.0)

        # # Get scale (can be uniform or separate X/Y)
        scale_x = obj.get("tex_stretch", 1.0)
        scale_y = obj.get("tex_stretch", 1.0)

        # Get offset (world position offset for triplanar)
        offset_x = obj.get("tex_offset_x", 0.0)
        offset_y = obj.get("tex_offset_y", 0.0)
    ...
    def execute(self, context):
        ...
        for prop in world.keys():
            if not world.bl_rna.properties.get(prop):
...
def debugprint(text, decor='#'):
    amount = []
    for i in range(20):  # Fixed: use range(20)
        amount.append(decor)

    # Join the list into a single string
    decor_line = ''.join(amount)

    print(decor_line)  # Print the full line of decor characters
    print(text)
    print(decor_line)
...
def add_player_node(node_group):
    ...
    node_group.nodes["Scale Elements"].inputs[4].default_value = 'Uniform'
    ...
    node_group.nodes["Scale Elements.001"].inputs[4].default_value = 'Uniform'
    ...
    node_group.nodes["Transform Geometry"].inputs[2].default_value[0] = 1.6
    node_group.nodes["Transform Geometry"].inputs[2].default_value[2] = 2.08
    node_group.nodes["Transform Geometry"].inputs[3].default_value[1] = 1.5708
...
def add_ent_node(node_group):
    ...
    trans_node = node_group.nodes.new('GeometryNodeTransform')
    node_group.nodes["Transform Geometry"].inputs[2].default_value[0] = 2.08
    node_group.nodes["Transform Geometry"].inputs[3].default_value[1] = 1.5708
...
def add_item_node(node_group):
    ...
    trans_node = node_group.nodes.new('GeometryNodeTransform')
    node_group.nodes["Transform Geometry"].inputs[2].default_value[0] = 2.03
    node_group.nodes["Transform Geometry"].inputs[2].default_value[2] = 1.8
    node_group.nodes["Transform Geometry"].inputs[3].default_value[1] = 1.5708
...
def add_init_convex_hull_node(node_group, object_list):
    ...
    for obj in object_list:
        ...
        object_info_node.inputs[0].default_value = obj
        object_info_node.transform_space = 'RELATIVE'
        node_group.links.new(object_info_node.outputs['Geometry'], join_geometry_node.inputs['Geometry'])



    node_group.links.new(convex_hull_node.outputs['Convex Hull'], output_node.inputs['Geometry'])
    node_group.links.new(convex_hull_node.inputs['Geometry'], join_geometry_node.outputs['Geometry'])
    node_group.links.new(input_node.outputs['Geometry'], join_geometry_node.inputs['Geometry'])

    return node_group

def add_convex_hull_node(node_group):
    x_panel = node_group.interface.new_panel(name="X")
    y_panel = node_group.interface.new_panel(name="Y")
    z_panel = node_group.interface.new_panel(name="Z")


    x_loc = node_group.interface.new_socket("X Location", socket_type="NodeSocketVector", in_out='INPUT', parent=x_panel)
    x_loc.dimensions = 2
    x_loc.structure_type = 'SINGLE'

    x_rot = node_group.interface.new_socket("X Angle", socket_type="NodeSocketFloat", in_out='INPUT', parent=x_panel)
    x_rot.structure_type = 'SINGLE'
    x_scale = node_group.interface.new_socket("X Scale", socket_type="NodeSocketFloat", in_out='INPUT', parent=x_panel)
    x_scale.structure_type = 'SINGLE'
    x_scale.default_value = 1.0

    y_loc = node_group.interface.new_socket("Y Location", socket_type="NodeSocketVector", in_out='INPUT', parent=y_panel)
    y_loc.dimensions = 2
    y_loc.structure_type = 'SINGLE'
    y_rot = node_group.interface.new_socket("Y Angle", socket_type="NodeSocketFloat", in_out='INPUT', parent=y_panel)
    y_rot.structure_type = 'SINGLE'
    y_scale = node_group.interface.new_socket("Y Scale", socket_type="NodeSocketFloat", in_out='INPUT', parent=y_panel)
    y_scale.structure_type = 'SINGLE'
    y_scale.default_value = 1.0

    z_loc = node_group.interface.new_socket("Z Location", socket_type="NodeSocketVector", in_out='INPUT', parent=z_panel)
    z_loc.dimensions = 2
    z_loc.structure_type = 'SINGLE'
    z_rot = node_group.interface.new_socket("Z Angle", socket_type="NodeSocketFloat", in_out='INPUT', parent=z_panel)
    z_rot.structure_type = 'SINGLE'
    z_scale = node_group.interface.new_socket("Z Scale", socket_type="NodeSocketFloat", in_out='INPUT', parent=z_panel)
    z_scale.structure_type = 'SINGLE'
    z_scale.default_value = 1.0
    ...
    store_node.data_type = 'FLOAT2'
    store_node.domain = 'CORNER'

    store_node.inputs[2].default_value = 'UVMap'

    pos_node = node_group.nodes.new('GeometryNodeInputPosition')
    normal_node = node_group.nodes.new('GeometryNodeInputNormal')

    sep_node1 = node_group.nodes.new('ShaderNodeSeparateXYZ')
    sep_node2 = node_group.nodes.new('ShaderNodeSeparateXYZ')

    combine_node1 = node_group.nodes.new('ShaderNodeCombineXYZ')
    multiply_node1 = node_group.nodes.new('ShaderNodeVectorMath')
    multiply_node1.operation = 'MULTIPLY'
    multiply_node1.inputs[1].default_value[0] = 1
    multiply_node1.inputs[1].default_value[1] = -1
    multiply_node1.inputs[1].default_value[2] = -1
    addition_node1 = node_group.nodes.new('ShaderNodeVectorMath')
    addition_node1.operation = 'ADD'
    scale_node1 = node_group.nodes.new('ShaderNodeVectorMath')
    scale_node1.operation = 'SCALE'
    rot_node1 = node_group.nodes.new('ShaderNodeVectorRotate')
    rot_node1.invert = True
    rot_node1.rotation_type = 'Z_AXIS'

    node_group.links.new(multiply_node1.outputs['Vector'], addition_node1.inputs[0])
    node_group.links.new(addition_node1.outputs['Vector'], scale_node1.inputs[0])
    node_group.links.new(scale_node1.outputs['Vector'], rot_node1.inputs[0])
    node_group.links.new(combine_node1.outputs['Vector'], addition_node1.inputs[1])
    node_group.links.new(input_node.outputs['Z Location'], multiply_node1.inputs[0])
    node_group.links.new(input_node.outputs['Z Angle'], rot_node1.inputs['Angle'])
    node_group.links.new(input_node.outputs['Z Scale'], scale_node1.inputs['Scale'])

    combine_node2 = node_group.nodes.new('ShaderNodeCombineXYZ')
    multiply_node2 = node_group.nodes.new('ShaderNodeVectorMath')
    multiply_node2.operation = 'MULTIPLY'
    multiply_node2.inputs[1].default_value[0] = -1
    multiply_node2.inputs[1].default_value[1] = -1
    multiply_node2.inputs[1].default_value[2] = -1
    addition_node2 = node_group.nodes.new('ShaderNodeVectorMath')
    addition_node2.operation = 'ADD'
    scale_node2 = node_group.nodes.new('ShaderNodeVectorMath')
    scale_node2.operation = 'SCALE'
    rot_node2 = node_group.nodes.new('ShaderNodeVectorRotate')
    rot_node2.invert = True
    rot_node2.rotation_type = 'Z_AXIS'

    node_group.links.new(multiply_node2.outputs['Vector'], addition_node2.inputs[0])
    node_group.links.new(addition_node2.outputs['Vector'], scale_node2.inputs[0])
    node_group.links.new(scale_node2.outputs['Vector'], rot_node2.inputs[0])
    node_group.links.new(combine_node2.outputs['Vector'], addition_node2.inputs[1])
    node_group.links.new(input_node.outputs['X Location'], multiply_node2.inputs[0])
    node_group.links.new(input_node.outputs['X Angle'], rot_node2.inputs['Angle'])
    node_group.links.new(input_node.outputs['X Scale'], scale_node2.inputs['Scale'])

    combine_node3 = node_group.nodes.new('ShaderNodeCombineXYZ')
    multiply_node3 = node_group.nodes.new('ShaderNodeVectorMath')
    multiply_node3.operation = 'MULTIPLY'
    multiply_node3.inputs[1].default_value[0] = -1
    multiply_node3.inputs[1].default_value[1] = -1
    multiply_node3.inputs[1].default_value[2] = -1
    addition_node3 = node_group.nodes.new('ShaderNodeVectorMath')
    addition_node3.operation = 'ADD'
    scale_node3 = node_group.nodes.new('ShaderNodeVectorMath')
    scale_node3.operation = 'SCALE'
    rot_node3 = node_group.nodes.new('ShaderNodeVectorRotate')
    rot_node3.invert = True
    rot_node3.rotation_type = 'Z_AXIS'

    node_group.links.new(multiply_node3.outputs['Vector'], addition_node3.inputs[0])
    node_group.links.new(addition_node3.outputs['Vector'], scale_node3.inputs[0])
    node_group.links.new(scale_node3.outputs['Vector'], rot_node3.inputs[0])
    node_group.links.new(combine_node3.outputs['Vector'], addition_node3.inputs[1])
    node_group.links.new(input_node.outputs['Y Location'], multiply_node3.inputs[0])
    node_group.links.new(input_node.outputs['Y Angle'], rot_node3.inputs['Angle'])
    node_group.links.new(input_node.outputs['Y Scale'], scale_node3.inputs['Scale'])

    mix_node1 = node_group.nodes.new('ShaderNodeMix')
    mix_node1.data_type = 'VECTOR'

    mix_node2 = node_group.nodes.new('ShaderNodeMix')
    mix_node2.data_type = 'VECTOR'

    abs_node1 = node_group.nodes.new('ShaderNodeMath')
    abs_node1.operation = 'ABSOLUTE'

    abs_node2 = node_group.nodes.new('ShaderNodeMath')
    abs_node2.operation = 'ABSOLUTE'

    great_node1 = node_group.nodes.new('ShaderNodeMath')
    great_node1.operation = 'GREATER_THAN'
    great_node1.inputs[1].default_value = 0.620

    great_node2 = node_group.nodes.new('ShaderNodeMath')
    great_node2.operation = 'GREATER_THAN'
    great_node2.inputs[1].default_value = 0.710
    ...
    ###
    node_group.links.new(join_geometry_node.outputs['Geometry'], store_node.inputs['Geometry'])
    ###

    node_group.links.new(store_node.outputs['Geometry'], output_node.inputs['Geometry'])

    #node_group.links.new(pos_node.outputs['Position'], scale_node.inputs['Vector'])
    #node_group.links.new(input_node.outputs['Scale'], scale_node.inputs['Scale'])
    #node_group.links.new(scale_node.outputs['Vector'], sep_node1.inputs['Vector'])

    node_group.links.new(sep_node1.outputs[0], combine_node1.inputs[0])
    node_group.links.new(sep_node1.outputs[0], combine_node3.inputs[0])
    node_group.links.new(sep_node1.outputs[1], combine_node1.inputs[1])
    node_group.links.new(sep_node1.outputs[1], combine_node2.inputs[0])
    node_group.links.new(sep_node1.outputs[2], combine_node2.inputs[1])
    node_group.links.new(sep_node1.outputs[2], combine_node3.inputs[1])

    node_group.links.new(rot_node1.outputs['Vector'], mix_node1.inputs['A'])
    node_group.links.new(rot_node2.outputs['Vector'], mix_node1.inputs['B'])
    node_group.links.new(rot_node3.outputs['Vector'], mix_node2.inputs['B'])
    node_group.links.new(mix_node1.outputs['Result'], mix_node2.inputs['A'])

    node_group.links.new(mix_node2.outputs['Result'], store_node.inputs['Value'])
    #node_group.links.new(rot_node1.inputs['Angle'], input_node.outputs['Angle'])
    #node_group.links.new(rot_node1.outputs['Vector'], store_node.inputs['Value'])

    node_group.links.new(normal_node.outputs['Normal'], sep_node2.inputs[0])
    node_group.links.new(sep_node2.outputs[0], abs_node1.inputs['Value'])
    node_group.links.new(sep_node2.outputs[1], abs_node2.inputs['Value'])

    node_group.links.new(abs_node1.outputs['Value'], great_node1.inputs['Value'])
    node_group.links.new(abs_node2.outputs['Value'], great_node2.inputs['Value'])
    node_group.links.new(great_node1.outputs['Value'], mix_node1.inputs['Factor'])
    node_group.links.new(great_node2.outputs['Value'], mix_node2.inputs['Factor'])

    node_group.links.new(pos_node.outputs[0], sep_node1.inputs['Vector'])
    return node_group

def update_sockets(obj, name):
    obj.modifiers[name]["Socket_13"] = 1.0
    obj.modifiers[name]["Socket_7"] = 1.0
    obj.modifiers[name]["Socket_10"] = 1.0

def add_geonode_to_object(obj, name, node, context):
            # Check if the object already has a Geometry Node modifier
                ...
                if modifier is None:
                    ...
                    if modifier.node_group is None:
                        ...
                        if node == "convex_hull":
                            node_tree = add_convex_hull_node(modifier.node_group)
                        elif node == "player":
                            node_tree = add_player_node(modifier.node_group)
                        elif node == "ent":
                            node_tree = add_ent_node(modifier.node_group)
                        elif node == "item":
                            node_tree = add_item_node(modifier.node_group)
                        elif node == "init_convex_hull":
                            node_tree = add_init_convex_hull_node(modifier.node_group, context)
...
class DuplicateObjectOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.scene.trenchcoat_props.invasive_mirror:
        ...
        for obj in selectedobjects:
            ...
            axis_block = []
            if context.scene.trenchcoat_props.mirror_axis == "x":
                axis_block = (True, False, False)
            elif context.scene.trenchcoat_props.mirror_axis == "y":
                axis_block = (False, True, False)
            elif context.scene.trenchcoat_props.mirror_axis == "z":
...
class OBJECT_OT_snap_selected_to_grid(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        grid_scale = float(bpy.context.scene.trenchcoat_props.grid_size)
        ...
        for obj in selected_objects:
            ...
            if object_results['misaligned_verts'] > 0:
                ...
                if context.scene.trenchcoat_props.snap_alone == False:
                    bpy.ops.mesh.select_all(action='SELECT')
...
class TCShortcuts_XRAY(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not context.scene.trenchcoat_props.shrt_xray:
            context.space_data.shading.show_xray = True
            context.scene.trenchcoat_props.shrt_xray = True
        else:
            context.space_data.shading.show_xray = False
            context.scene.trenchcoat_props.shrt_xray = False
...
class TCShortcuts_OBJCOL(bpy.types.Operator):
    ...
    def execute(self, context):
        if not context.scene.trenchcoat_props.shrt_obj_col:
            context.space_data.overlay.show_retopology = True
            context.scene.trenchcoat_props.shrt_obj_col = True
        else:
            context.space_data.overlay.show_retopology = False
            context.scene.trenchcoat_props.shrt_obj_col = False
        return {'FINISHED'}
...
class TCShortcuts_PIVOTCURSOR(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not context.scene.trenchcoat_props.shrt_pivot_cursor:
            context.scene.tool_settings.transform_pivot_point = 'CURSOR'

            context.scene.trenchcoat_props.shrt_pivot_cursor = True
        else:
            context.scene.tool_settings.transform_pivot_point = 'MEDIAN_POINT'
            context.scene.trenchcoat_props.shrt_pivot_cursor = False
        return {'FINISHED'}
...
class TCShortcuts_INCREMENT(bpy.types.Operator):
    ...
    def execute(self, context):
        bpy.context.space_data.overlay.grid_scale = float(bpy.context.scene.trenchcoat_props.grid_size)
        if bpy.context.scene.tool_settings.snap_elements_base != {'INCREMENT'} or bpy.context.scene.tool_settings.use_snap == False:
            ...
            context.scene.trenchcoat_props.snapset1 = True
            context.scene.trenchcoat_props.snapset2 = False
            context.scene.trenchcoat_props.snapset3 = False
        else:
            bpy.context.scene.tool_settings.use_snap = False
            bpy.context.scene.tool_settings.snap_elements_base = {'VERTEX', 'EDGE', 'EDGE_MIDPOINT'}
            context.scene.trenchcoat_props.snapset1 = False

        return {'FINISHED'}

class TCShortcuts_VERTSNAP(bpy.types.Operator):
    bl_idname = "object.trenchcoat_shortcuts_vert"
    bl_label = "Trenchcoat Shortcuts3"
    bl_description = "Toggles vertsnapping"
    bl_options = {'UNDO'}

    def execute(self, context):
        bpy.context.space_data.overlay.grid_scale = float(bpy.context.scene.trenchcoat_props.grid_size)
        if bpy.context.scene.tool_settings.snap_elements_base != {'VERTEX'}:
            bpy.context.scene.tool_settings.snap_elements_base = {'VERTEX'}
            bpy.context.scene.tool_settings.use_snap_nonedit = True
            bpy.context.scene.tool_settings.use_snap = False
            context.scene.trenchcoat_props.snapset1 = False
            context.scene.trenchcoat_props.snapset2 = True
            context.scene.trenchcoat_props.snapset3 = False
        else:
            bpy.context.scene.tool_settings.use_snap = False
            bpy.context.scene.tool_settings.snap_elements_base = {'VERTEX', 'EDGE', 'EDGE_MIDPOINT'}
            bpy.context.scene.tool_settings.use_snap_nonedit = False
            context.scene.trenchcoat_props.snapset2 = False

        return {'FINISHED'}

class TCShortcuts_EDGE_SNAP(bpy.types.Operator):
    bl_idname = "object.trenchcoat_shortcuts_edge"
    bl_label = "Trenchcoat Shortcuts2"
    bl_description = "Toggles edgesnapping"
    bl_options = {'UNDO'}

    def execute(self, context):
        bpy.context.space_data.overlay.grid_scale = float(bpy.context.scene.trenchcoat_props.grid_size)
        if bpy.context.scene.tool_settings.snap_elements_base != {'EDGE'}:
            bpy.context.scene.tool_settings.snap_elements_base = {'EDGE'}
            bpy.context.scene.tool_settings.use_snap_nonedit = True
            bpy.context.scene.tool_settings.use_snap = False
            context.scene.trenchcoat_props.snapset1 = False
            context.scene.trenchcoat_props.snapset2 = False
            context.scene.trenchcoat_props.snapset3 = True
        else:
            bpy.context.scene.tool_settings.use_snap = False
            bpy.context.scene.tool_settings.snap_elements_base = {'VERTEX', 'EDGE', 'EDGE_MIDPOINT'}
            bpy.context.scene.tool_settings.use_snap_nonedit = False
...
class CalcGridstance(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        grid_scale = float(context.scene.trenchcoat_props.grid_size)
...
#         }
        context.scene.trenchcoat_props.gridsteps = total_grid_steps
        return {'FINISHED'}

class ConvexHullBrush(bpy.types.Operator):
    bl_idname = "object.convexhull_brush"
    bl_label = "Make Brush"
    bl_description = "Creates a convex hull brush from selected objects"
    bl_options = {'UNDO'}

    def execute(self, context):
        active_object = context.active_object
        selected_objects = context.selected_objects
        material = context.scene.trenchcoat_props.material
        ...
        if context.scene.trenchcoat_props.automerge:
            # Add geometry nodes to active object with all selected objects as inputs
            add_geonode_to_object(active_object, "InitConvexHull", "init_convex_hull", selected_objects)

            # Get objects to delete (all selected except active)
            to_delete = [obj for obj in selected_objects if obj != active_object]

            # Make sure active object is still selected and active
            active_object.select_set(True)
            context.view_layer.objects.active = active_object

            # Convert to mesh (applies geometry nodes modifier)
            bpy.ops.object.convert(target='MESH')
            try:
                delete_me = bpy.data.node_groups.get("InitConvexHullGeometryGroup")
                bpy.data.node_groups.remove(delete_me)
            except:
                pass
            ...
            # Delete faces and edges (keep only vertices)
            bpy.ops.mesh.delete(type='EDGE_FACE')

            # Exit edit mode
            bpy.ops.object.mode_set(mode='OBJECT')

            add_geonode_to_object(active_object, "ConvexHullBrush", "convex_hull", selected_objects)
            update_sockets(active_object, "ConvexHullBrush")
            active_object.name = 'brush'
            if active_object.data.materials:
                # Object already has materials, assign to first slot
                active_object.data.materials[0] = material
            else:
                # Object has no materials, create first slot
            ...
            for obj in selected_objects:
                ...
                add_geonode_to_object(obj, "ConvexHullBrush", "convex_hull", selected_objects)
                update_sockets(obj, "ConvexHullBrush")

            bpy.ops.object.select_all(action='DESELECT')
            for obj in selected_objects:
                obj.select_set(True)
            context.view_layer.objects.active = active_object
            self.report({'INFO'}, f"Created brush: {active_object.name}")

        bpy.ops.object.mode_set(mode=original_mode)
        return {'FINISHED'}

class FillWithBrush(bpy.types.Operator):
    bl_idname = "object.fill_with_brush"
    bl_label = "Fill with Brush"
    bl_description = "Fill selected verts with a brush"
    bl_options = {'UNDO'}
    ...
    def execute(self, context):
        ...
        # Check if we're in object mode or edit mode
        if context.mode == 'OBJECT':
            # OBJECT MODE: Spawn a cube at 3D cursor
            bpy.ops.mesh.primitive_cube_add(location=context.scene.cursor.location)
            new_obj = context.active_object

            # Convert to brush (clean to verts only + add convex hull nodegroup)
            self.convert_to_brush(context, new_obj)
            update_sockets(new_obj, "ConvexHullBrush")
            new_obj.name = 'brush'
            new_obj.data.materials.append(material)

            self.report({'INFO'}, f"Spawned brush: {new_obj.name}")

        elif context.mode == 'EDIT_MESH':
            # EDIT MODE: Duplicate selected verts, separate, convert to brush

            # Check if anything is selected in edit mode
            ...
            # Store the original mesh object
            original_obj = active_object
            non_selectable = original_obj.hide_select
            if non_selectable:
                original_obj.hide_select = False

            # Duplicate selected vertices
            bpy.ops.mesh.duplicate()

            # Separate the duplicated vertices into a new object
            bpy.ops.mesh.separate(type='SELECTED')

            # Exit edit mode to access the new object
            bpy.ops.object.mode_set(mode='OBJECT')

            # Get the new object (selected objects minus the active one)
            new_obj = None
            ...
            if not new_obj:
                self.report({'WARNING'}, "No new object created from separation")
                return {'CANCELLED'}


            if non_selectable:
                original_obj.hide_select = True

            context.view_layer.objects.active = new_obj

            # Check if less than 3 vertices were selected
            if len(selected_verts) < 3:
                # Convert to brush but stay in edit mode
                new_obj.modifiers.clear()
                self.convert_to_brush2(context, new_obj, stay_in_edit_mode=True)
                update_sockets(new_obj, "ConvexHullBrush")
                new_obj.name = 'brush'
                ...
                # Stay in edit mode on the new brush
                bpy.ops.object.mode_set(mode='EDIT')
                bpy.ops.mesh.select_all(action='SELECT')

                self.report({'INFO'}, f"Created brush from {len(selected_verts)} vertices - edit it to add more")
                return {'FINISHED'}

            # Normal case: 3+ vertices selected
            new_obj.modifiers.clear()
            self.convert_to_brush(context, new_obj)
            update_sockets(new_obj, "ConvexHullBrush")
            new_obj.name = 'brush'
            ...
            # Go back to the original mesh and enter edit mode
            original_obj.select_set(True)
            context.view_layer.objects.active = original_obj
            bpy.ops.object.mode_set(mode='EDIT')

            self.report({'INFO'}, f"Created brush from selection: {new_obj.name}")
    ...
    def convert_to_brush2(self, context, obj, stay_in_edit_mode=False):
        """Convert object to brush (verts only + convex hull)"""
        ...
        # If we want to stay in edit mode, don't exit
        if not stay_in_edit_mode:
            bpy.ops.object.mode_set(mode='OBJECT')

        # Add geometry nodes
        if stay_in_edit_mode:
            # Need to exit to add modifier, then go back
            bpy.ops.object.mode_set(mode='OBJECT')
            add_geonode_to_object(obj, "ConvexHullBrush", "convex_hull", [obj])
            # The caller will handle going back to edit mode
        else:
            add_geonode_to_object(obj, "ConvexHullBrush", "convex_hull", [obj])

        return obj

    def convert_to_brush(self, context, obj):
        """Helper function: convert object to brush (verts only + convex hull nodegroup)"""
        ...
        # Add geometry nodes to this object
        add_geonode_to_object(obj, "ConvexHullBrush", "convex_hull", [obj])

        return obj

class Select_Bug(bpy.types.Operator):
    bl_idname = "object.select_bug"
    bl_label = "Select Bug"
    bl_description = "Selects objects that are non-manifold"
    bl_options = {'UNDO'}

    def execute(self, context):
        # Get the threshold from scene property
        threshold = context.scene.trenchcoat_props.numberofverts
        ...
        for obj in context.scene.objects:
            if obj.type == 'MESH':
                ...
                # Select if at or below threshold
        ...
        # Report results
        if selected_count == 0:
            self.report({'INFO'}, f"No objects found with {threshold} or fewer vertices")
        else:
            self.report({'INFO'}, f"Selected {selected_count} objects with {threshold} or fewer vertices")

        return {'FINISHED'}

class SplitBrush(bpy.types.Operator):
    bl_idname = "object.split_brush"
    bl_label = "Split Brush"
    bl_description = "Split the selected brush along the 3D cursor's Y axis"
    bl_options = {'UNDO'}

    def execute(self, context):
        active_object = context.active_object

        if not active_object or active_object not in context.selected_objects:
            self.report({'WARNING'}, "No active object")
            return {'CANCELLED'}

        try:
            if active_object.modifiers["ConvexHullBrush"]:
        ...
        bpy.ops.object.mode_set(mode='OBJECT')
        bpy.ops.object.convert(target='MESH')
        # Get cursor location and rotation
        cursor_loc = context.scene.cursor.location
        cursor_rot = context.scene.cursor.rotation_euler.to_matrix()

        # Cursor's local Y axis in world space
        if context.scene.trenchcoat_props.split_axis == 'x':
            cursor_y_axis = cursor_rot @ Vector((0, 1, 0))
        else:
            cursor_y_axis = cursor_rot @ Vector((1, 0, 0))

        # Duplicate the object
        obj_copy = active_object.copy()
        obj_copy.data = active_object.data.copy()
        context.collection.objects.link(obj_copy)

        # --- Process Original (Keep Negative Y side relative to cursor) ---
        bpy.ops.object.select_all(action='DESELECT')
        active_object.select_set(True)
        context.view_layer.objects.active = active_object
        ...
        bpy.ops.mesh.bisect(
            plane_co=cursor_loc,
            plane_no=cursor_y_axis,  # Cursor's Y axis
            use_fill=True,
            clear_inner=False,
            clear_outer=True   # Delete positive Y side
        )
        bpy.ops.object.mode_set(mode='OBJECT')

        # Convert to brush
        self.convert_to_brush(context, active_object)

        # --- Process Copy (Keep Positive Y side relative to cursor) ---
        bpy.ops.object.select_all(action='DESELECT')
        ...
        bpy.ops.mesh.bisect(
            plane_co=cursor_loc,
            plane_no=-cursor_y_axis,  # Negative cursor Y axis
            use_fill=True,
            clear_inner=False,
            clear_outer=True   # Delete negative Y side
        )
        bpy.ops.object.mode_set(mode='OBJECT')

        # Convert to brush
        self.convert_to_brush(context, obj_copy)

        # Select both new brushes
        bpy.ops.object.select_all(action='DESELECT')
        active_object.select_set(True)
        obj_copy.select_set(True)
        for obj in context.selected_objects:
        ...
        self.report({'INFO'}, f"Split {active_object.name} into two brushes")
        return {'FINISHED'}
...
class ApplyMaterial(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for obj in context.selected_objects:
            if obj.type == 'MESH':
                if context.scene.trenchcoat_props.material is not None:
                    obj.data.materials.clear()
                    obj.data.materials.append(context.scene.trenchcoat_props.material)
                    try:
                        update_sockets(obj, "ConvexHullBrush")
                    except:
                        pass
...
class DuplicateMaterial(bpy.types.Operator):
    ...
    new_name: bpy.props.StringProperty(
        name="New Name",
        description="Name for the duplicated material",
        default=""
    )
    ...
    def execute(self, context):
        ...
        # Set the new name if provided, otherwise use default
        if self.new_name:
            new_material.name = self.new_name
        else:
            new_material.name = material.name + "_copy"

        # Optionally select the new material
        context.scene.trenchcoat_props.material = new_material

        self.report({'INFO'}, f"Material duplicated as '{new_material.name}'")
        return {'FINISHED'}
    ...
    def invoke(self, context, event):
        ...
        # Set default name in the dialog
        self.new_name = material.name + "_copy"
...
class DeleteMaterial(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, f"Material '{material_name}' deleted")
        return {'FINISHED'}
    ...
    def invoke(self, context, event):
        ...
        # Show simple OK/Cancel confirmation
    ...
    def draw(self, context):
        ...
        if material:
            layout.label(text=f"Delete '{material.name}'?")
            if material.users > 0:
                layout.label(text=f"⚠️ Used by {material.users} object(s)", icon='ERROR')
    ...
    def execute(self, context):
        ...
        if list_of_things == []:
            ...
            if bpy.context.object.mode == 'EDIT':
                for obj in bpy.data.objects:
                    if obj != context.edit_object and obj != context.scene.trenchcoat_props.frame_object and obj not in context.selected_objects:
...
class Editframe(bpy.types.Operator):
    bl_idname = "object.edit_frame"
    bl_label = "Edit Frame"
    bl_description = "Edit the frame"
    bl_options = {'UNDO'}

    def execute(self, context):
        # Get the frame object from scene
        obj = context.scene.trenchcoat_props.frame_object

        # Ensure we have a valid object
        if not obj:
            self.report({'ERROR'}, "No frame object selected")
            return {'CANCELLED'}
...
class CreateEntityCube(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        grid_size = float(context.scene.trenchcoat_props.grid_size)
        original_grid = bpy.context.space_data.overlay.grid_scale
        ...
        if self.mode == 0:
            ...
            add_geonode_to_object(mesh_obj, "GenericEntGen", "ent", context)
        ...
        if self.mode == 2:
            ...
            mesh_obj.select_set(True)
            add_geonode_to_object(mesh_obj, "PlayerEntGen", "player", context)
        ...
        if self.mode == 1:
            ...
            mesh_obj.select_set(True)
            add_geonode_to_object(mesh_obj, "GenericItemGen", "item", context)
...
class OBJECT_OT_AddPropertyFromText(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        text_input = context.scene.trenchcoat_props.text_of_prop.strip()
        if not text_input:
        ...
        if text_input.lower().startswith('get '):
            ...
            if prop_name and prop_name in active_obj:
                ...
                context.scene.trenchcoat_props.text_of_prop = value
        ...
        for line in text_input.split('\n'):
            ...
            if text_input.startswith('!') and (',' in text_input or ':' in text_input):
                ...
                context.scene.trenchcoat_props.text_of_prop = ""
...
class MESH_OT_add_bounding_box_vertices(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        obj = context.active_object
        grid_size = float(context.scene.trenchcoat_props.grid_size)
        snap = context.scene.trenchcoat_props.snap
        original_grid = bpy.context.space_data.overlay.grid_scale
        ...
        if obj is None or obj.type != 'MESH':
            ...
            return {'CANCELLED'}
        try:
            if obj.modifiers["ConvexHullBrush"]:
                if obj.data.materials and len(obj.data.materials) > 0:
                    material = obj.data.materials[0]
                update_sockets(obj, "ConvexHullBrush")
        except:
...
class MESH_PT_actual_vertex_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout = self.layout
        obj = context.active_object
        snapset1 = context.scene.trenchcoat_props.snapset1
        snapset2 = context.scene.trenchcoat_props.snapset2
        snapset3 = context.scene.trenchcoat_props.snapset3
        gridsteps = context.scene.trenchcoat_props.gridsteps
        ...
        if obj is not None:
            ...
            row.prop(context.scene.trenchcoat_props, "grid_size", text = "")
            if context.scene.trenchcoat_props.snap:
                row.prop(context.scene.trenchcoat_props, "snap", text = "", icon="SNAP_ON", toggle=True)
            else:
                row.prop(context.scene.trenchcoat_props, "snap", text = "", icon="SNAP_OFF", toggle=True)
            if bpy.context.object.mode == 'EDIT':
                row = box.row(align=True)
                row.operator("object.snap_selected_to_grid", text = "Snap Vertices to Grid", icon="SNAP_GRID")
                if context.scene.trenchcoat_props.snap_alone:
                    row.prop(context.scene.trenchcoat_props, "snap_alone", text = "", icon="TRACKER", toggle=True)
                else:
                    row.prop(context.scene.trenchcoat_props, "snap_alone", text = "", icon="TRACKER", toggle=True)
            ...
            if context.selected_objects:
                ...
                row.operator("object.fill_with_brush", text = "Spawn Brush", icon="SNAP_VOLUME")
            #row.prop(context.scene, "hintcage", text = "", icon="MESH_ICOSPHERE", toggle=True)
            if context.scene.trenchcoat_props.automerge:
                row.prop(context.scene.trenchcoat_props, "automerge", text = "", icon="AUTOMERGE_ON", toggle=True)
            else:
                row.prop(context.scene.trenchcoat_props, "automerge", text = "", icon="AUTOMERGE_OFF", toggle=True)
            row = box.row(align=True)
            row.prop(context.scene.trenchcoat_props, "material", text = "")
            row.operator("object.apply_material", text = "", icon="MATERIAL")
            row.operator("object.duplicate_material", text = "", icon="DUPLICATE")
            row.operator("object.delete_material", text = "", icon="TRASH")

            if bpy.context.object.mode == 'OBJECT':
                row = box.row(align=True)
                row.label(text="Spawn:")
                row.operator("object.create_entity_cube", text = "+Player", icon="OUTLINER_OB_ARMATURE").mode = 2
                row.operator("object.create_entity_cube", text = "+Item", icon="GEOMETRY_SET").mode = 1
                row.operator("object.create_entity_cube", text = "+Entity", icon="TRACKER").mode = 0
                ...
                if len(context.selected_objects) > 0 and len([o for o in context.selected_objects if o.type == 'MESH']) == len(context.selected_objects):
                    ...
                    split.prop(context.scene.trenchcoat_props, "mirror_axis", text = "")
                    split = row.split(factor=0.55)
                    split.prop(context.scene.trenchcoat_props, "invasive_mirror", text = "", icon="SNAP_PEEL_OBJECT", toggle=True)
            row = layout.row(align=True)
            ...
            if bpy.context.object.mode == 'OBJECT':
                ...
                split.operator("object.fill_with_brush", text = "Fill", icon="MOD_SOLIDIFY")
            row = layout.row(align=True)
            row.operator('object.split_brush', text = 'Split')
            row.prop(context.scene.trenchcoat_props, "split_axis", text = "")
            box2 = layout.box()
            row = box2.row()
        ...
        row = layout.row(align=True)
        row.prop(context.scene.trenchcoat_props, "frame_object", text = "Frame")
        row.operator("object.edit_frame", text = "", icon="EDITMODE_HLT")
        row.operator("object.popfront", text = "", icon="ZOOM_SELECTED").use_frame = True
        if context.scene.trenchcoat_props.frame_object:
            row.prop(context.scene.trenchcoat_props.frame_object, 'hide_select', text='', icon="RESTRICT_SELECT_OFF")
        else:
            pass
        #row.operator make it infornt
        #row.operator edit frame
        #row.operator edit with another brush, make them solo...
        #row.operator. lock it in place

        row = layout.row(align=True)
        if context.scene.trenchcoat_props.shrt_obj_col:
            row.alert = True
        else:
            row.alert = False
        row.operator("object.trenchcoat_shortcuts_objcol", text = "NoHull", icon="GHOST_ENABLED")
        if context.scene.trenchcoat_props.shrt_xray:
            row.alert = True
        else:
            row.alert = False
        row.operator("object.trenchcoat_shortcuts_xray", text = "XRay", icon="MOD_WIREFRAME")
        if context.scene.trenchcoat_props.shrt_pivot_cursor:
            row.alert = True
        else:
            row.alert = False
        ...
        if is_in_front:
            split.alert = True
            split.operator("object.popfront", text = "In Front On/Off", icon="ZOOM_SELECTED").use_frame = False
        else:
            split.operator("object.popfront", text = "In Front On/Off", icon="ZOOM_SELECTED").use_frame = False
        split.alert = False
        ...
        if snapset1:
            split.alert = True
            if bpy.context.space_data.overlay.grid_scale == float(bpy.context.scene.trenchcoat_props.grid_size):
        ...
        split.alert = False
        split.operator("object.gridstancecalc", text=f"{context.scene.trenchcoat_props.gridsteps:.1f}", icon="DRIVER_DISTANCE")
        row = layout.row(align=True)
        if snapset2:
            row.alert = True
        row.operator('object.trenchcoat_shortcuts_vert', text='Snap Vert', icon='SNAP_VERTEX')
        row.alert = False
        if snapset3:
            row.alert = True
        row.operator('object.trenchcoat_shortcuts_edge', text='Snap Edge', icon ='SNAP_EDGE')
        row.alert = False
        row = layout.row(align=True)
        row.prop(context.scene.tool_settings, "use_transform_data_origin", text = "Only transform origin", icon='GIZMO')
        row = layout.row(align=True)
        row.label(text="Debug:")
        row = layout.row(align=True)
        split = row.split(factor=0.80)
        split.operator('object.select_bug', text = "Check non-manifold", icon = "STICKY_UVS_LOC")
        split.prop(context.scene.trenchcoat_props, "numberofverts", text = "")
        ...
        row = box.row(align=True)
        row.operator('object.convcol_suffix', text = "Convcol", icon="ADD").action = 'ADD'
        row.operator('object.convcol_suffix', text = "Convcol", icon="REMOVE").action = 'REMOVE'
        ...
        if context.active_object:
            ...
            row = layout.row(align=True)
            row.prop(context.scene.trenchcoat_props, "text_of_prop")
...
classes = (
    ...
    FillWithBrush,
    SplitBrush,
    TCShortcuts_VERTSNAP,
    TCShortcuts_EDGE_SNAP,
    Select_Bug,
    Editframe,
    Trenchcoat_Properties,
    OBJECT_OT_convcol_suffix

)

def menu_func_export(self, context):
    self.layout.operator(ExportQuakeMap.bl_idname, text="Quake Map (.map)")
    self.layout.operator(OBJECT_OT_export_with_convcol.bl_idname, text="glTF with .convcol Suffix")

def register():
    for cls in classes:
        bpy.utils.register_class(cls)
    bpy.types.TOPBAR_MT_file_export.append(menu_func_export)
    bpy.types.Scene.trenchcoat_props = bpy.props.PointerProperty(type=Trenchcoat_Properties)

def unregister():
    for cls in classes:
        bpy.utils.unregister_class(cls)
    bpy.types.TOPBAR_MT_file_export.remove(menu_func_export)
    del bpy.types.Scene.trenchcoat_props
